...
<head>
  ...
  <title>User / Update — GitHub style</title>
  ...
  <style>
    ...
    *{box-sizing:border-box}
    ...
    .btn{display:inline-flex;align-items:center;gap:8px;padding:8px 12px;border-radius:8px;font-weight:600;border:none;cursor:pointer}
...
<body>
  ...
  <main class="container">
    <div class="grid">
      <section class="card">
        <h2>Update user</h2>
        <p class="lead">Modify the account details below and save changes.</p>
        <form method="post" action="<?=site_url('user/update/'. $user['id']) ?>">
          <div class="field">
            <label for="username">Username</label>
            <input id="username" name="username" type="text" value="<?=html_escape($user['username'])?>" required>
          </div>
          <div class="field">
      ...
      <aside class="sidebar">
        <div class="card">
          <div style="display:flex;gap:12px;align-items:center">
            ...
            <div>
              <strong>Updating tips</strong>
              <div class="muted" style="margin-top:6px">Be sure the email is correct. Username changes might affect linked projects.</div>
            </div>
          </div>
          <hr style="margin:16px 0;border:none;border-top:1px solid rgba(255,255,255,0.03)">
          <div class="meta">
            <div><strong>Username rules</strong></div>
            <div class="muted">Only alphanumeric characters and hyphens. Max 39 characters.</div>
            <div style="margin-top:10px"><strong>Email format</strong></div>
            <div class="muted">Use a valid email address you have access to.</div>
          </div>
        </div>
      </aside>
    </div>
